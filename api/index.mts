...
app.post("/getBestNotes", async (req, res) => {
  try {
    ...
    console.log("Indices of highest score:", maxIndices);
    if (maxIndices[0] === -1) {
      return res.json({
        response: 0,
        ipfsNote: {
          comment: "No notes found for the given address",
          sentiment: false,
        },
      });
    }
    console.log(
      "Highest value? : ",
      response[maxIndices[0]][maxIndices[1]][maxIndices[2]]
    );

    // console.log('cid : ', response[maxIndex][0][2].cid)

    const noteFromCid = await axios.get(
      `https://${
        response[maxIndices[0]][maxIndices[1]][maxIndices[2]]
      }.ipfs.w3s.link`
    );
    ...
    res.json({
      response: Number(response[maxIndices[0]][maxIndices[1]]),
      ipfsNote: noteFromCid.data,
    });
    console.log("ipfsNote : ", noteFromCid.data);
  } catch (error) {
    console.error("Error retrieving note:", error);
    res.status(500).json({ message: "Failed to retrieve note", error });
  }
